...
T printArray( const T *array,int count,int lowSubscript,int highSubscript)
{
   ...
   if ( lowSubscript>highSubscript||lowSubscript<0||highSubscript>count/* Write conditions to test if the size if negative,
...
int main()
{
   ...
   elements = printArray( c, CCOUNT ,0,-2);/* Write a call to printArray that specifies
